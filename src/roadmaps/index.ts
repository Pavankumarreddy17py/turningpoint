...
const generator = roadmapGenerators[dreamCategory] || generateGenericRoadmap;
...
function generateSportsRoadmap(dreamName: string, responses: Record<string, string>) {
  const sportType = responses.sport_type || responses.outdoor_sport || responses.indoor_sport || 'sports';
  const practiceHours = responses.study_hours || responses.practice_time || '4-6';
  const rolePrefix = responses.cricket_role || responses.football_position || responses.hockey_position || responses.basketball_position || responses.volleyball_role || responses.tennis_style || responses.handball_position || responses.rugby_position || responses.polo_role || responses.kho_kho_role || responses.badminton_type || responses.tt_style || '';
  
  let exactRole = sportType;
  if (rolePrefix) {
    exactRole = `${rolePrefix} in ${sportType}`;
    if (responses.cricket_batting_style) exactRole += ` (${responses.cricket_batting_style} handed)`;
    if (responses.football_foot) exactRole += ` (Prefers ${responses.football_foot} foot)`;
    if (responses.tennis_surface) exactRole += ` on ${responses.tennis_surface} courts`;
  }

  const roleTitle = exactRole.split('_').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
  const sportTitle = sportType.split('_').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
...
function generateEducationRoadmap(dreamName: string, responses: Record<string, string>) {
  const level = responses.education_level || 'Higher Education';
  const field = responses.school_stream || responses.ug_type || responses.pg_field || responses.research_interest || 'General Studies';
  const specificGoal = responses.school_goal || responses.engineering_branch || responses.pg_goal || 'Academic Excellence';
  const studyHours = responses.study_hours || '4-6';
...
function generateBusinessRoadmap(dreamName: string, responses: Record<string, string>) {
  const businessType = responses.business_type || 'traditional';
  const investment = responses.investment_range || 'moderate';
